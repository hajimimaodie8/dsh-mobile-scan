...
fun PairScreen(
    ...
    /**
     * Open the camera as soon as this screen appears.
     *
     * Set by the scan-first launcher. With nothing remembered there is exactly one useful action
     * here, and requiring a tap to reach it is the friction this build removes. Cancelling still
     * leaves the typed form below, which stays the fallback.
     */
    ...
    // Scan-first entry: fires once for this appearance, and never when the caller supplied an
    // address to type against instead.
    LaunchedEffect(autoScan, prefillUrl) {
        if (autoScan && prefillUrl == null && state.paired == null) launchScan()
    }
