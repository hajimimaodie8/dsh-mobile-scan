 ...
 */
val dshVersionName: String = System.getenv("DSH_VERSION_NAME")?.takeIf { it.isNotBlank() } ?: "0.10.0"
...
android {
    ...
    signingConfigs {
        ...
        if (!keystore.isNullOrBlank() && file(keystore).exists()) {
            ...
            // Scan-variant: an unsigned APK cannot be installed at all, and this build is meant to
            // be installed straight from CI. Fall back to the debug key — it installs fine, it just
            // cannot replace an upstream-signed install in place (uninstall that one first). Set the
            // DSH_KEYSTORE* secrets to sign with a key of your own instead.
            buildTypes.getByName("release") {
